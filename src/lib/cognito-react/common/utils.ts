...
export function removeFragmentIdentifierFromBrowserLocation() {
  ...
  // Instead of using the configured history object, use the native browser API
  try {
    const currentUrl = currentBrowserLocationWithoutFragmentIdentifier();
    window.history.pushState(null, '', currentUrl);
  } catch (error) {
    console.warn('Failed to remove fragment identifier from URL:', error);
    // Fallback: just clear the hash without changing history
