...
import { generateStaticParamsForBusinesses, getInsightPost, getRelatedInsights } from "@/lib/insights";
...
export async function generateStaticParams() {
  return generateStaticParamsForBusinesses();
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const post = await getInsightPost('businesses', slug);
...
export default async function BusinessArticle({ params }: PageProps) {
  const { slug } = await params;
  const post = await getInsightPost('businesses', slug);
        ...
        <div className="container mx-auto px-4">
          <div className="mx-auto max-w-4xl">
            <Button variant="ghost" asChild className="mb-6">
              <Link href="/insights/businesses" className="flex items-center">
                ...
                {relatedPosts.map((relatedPost) => (
                  <div key={relatedPost.id} className="rounded-2xl border bg-card p-6 transition-all hover:shadow-lg">
                    ...
                    <Button variant="ghost" size="sm" asChild className="w-full">
                      <Link href={`/insights/businesses/${relatedPost.slug}`}>
                        Read Article
        ...
        <div className="container mx-auto px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mt-6 text-lg text-muted-foreground">
              Let our experts help you implement these strategies and navigate your digital transformation journey.
            </p>
